...
layer. The policy is a 12-action legs-only MLP over a stacked-5-deep
observation frame; the upper body is PD-held and is NOT actuated here, so this
...
Two observation generations ship, and the controller sniffs which one it holds
from the width of `actor.w_0` — they are not interchangeable weights:

  obs45 (v19/v21) = [last_action(12), cmd(4), q-default(12), qdot_fd(12),
                     projected_gravity(3), sin 2*pi*phi, cos 2*pi*phi]
  obs48 (v24)     = the identical 45 slots in the identical order, then
                    base angular velocity(3) at slots 45-47.
...
`velocity_flat.rs`) — every one of these is load-bearing:
  ...
  * the gyro slots are BODY frame, straight off the IMU with no rotation
...
Known limits of the default v24 checkpoint (see the model card):
  * **Yaw does not work.** +0.4 and -0.4 rad/s commands produce near-identical
    yaw in every engine tested. Do not command yaw and expect rotation.
  * **A zero command is a slow creep, not a stop** (~0.2 m/s). Releasing the
    stick does not stop this policy; ending the episode is what stops it.
  * Forward speed transfers at roughly 50-70% of the source engine's reading.
"""
...
# Action scale is per generation, not per robot: the 48-dim actors were trained
# at half the excursion of the 45-dim ones. Keyed by frame width so a single
# controller runs both checkpoints correctly.
ACTION_SCALE = {OBS_FRAME: 0.5, OBS_FRAME_GYRO: 0.25}
...
# Default Hub location for the released checkpoint. v24 is the current
# generation: it tracks commanded forward speed instead of collapsing to a
# single gait, plants the sole flat, and was fall-free across all four physics
# engines. Set ZEALOT_POLICY_FILE=g1_v21_iter4560.safetensors to fall back to
# the older 45-dim checkpoint that was flown on hardware.
DEFAULT_REPO_ID = "haixuantao/zealot-g1-locomotion"
DEFAULT_FILENAME = "g1_v24_iter32780.safetensors"
...
class _Policy:
    ...
    def __init__(self, path: str):
        ...
        # earlier, 48 = v24 with the gyro. Sniffed so one controller runs both,
        # and it carries the action scale with it (see ACTION_SCALE).
...
class ZealotLocomotionController:
    ...
    def run_step(self, action: dict, lowstate) -> dict:
        ...
        if self.policy.frame >= OBS_FRAME_GYRO:
            ...
            # what the v24 actor uses to close the loop on yaw; on hardware it
        ...
        target = np.clip(
            DEFAULT_LEG + self.policy.action_scale * policy_action,
            LEG_LIMITS[:, 0],
            LEG_LIMITS[:, 1],
        )
