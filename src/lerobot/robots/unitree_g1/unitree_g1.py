...
# Damping-mode release gain, matching Unitree's own create_damping_cmd (kp=0,
# kd=8 on every motor). The last low-level command persists on the robot, so a
# single damped message keeps the joints damped after we stop publishing.
DAMPING_KD = 8.0
...
class UnitreeG1(Robot):
    ...
    def _send_damping(self) -> None:
        ...
        This is Unitree's own safe-stop: with position gains off and only
        velocity damping left, the robot sinks under its own weight instead of
        free-falling the way a fully zero-gain command would drop it. It still
        does NOT hold itself up -- the operator must be carrying the weight.
        """
        try:
            with self._lowstate_lock:
                lowstate = self._lowstate
    ...
    def disconnect(self):
        # Stop the controller loop BEFORE sending the release command. The
        # 50 Hz loop republishes stiff gains continuously; releasing first
        # would let it overwrite the damping command in the gap.
        ...
        # Release into damping mode. Uses the last cached lowstate; with kp=0
        # the position targets are inert, only the damping matters.
